...
public class BaseMobile {
	...
	String browser = utilMobile.getConfigValue("browser");
	...
	/*
	 * @BeforeSuite public void extentSetup(ITestContext context) {
	 * ExtentManager.setOutputDirectory(context); extent =
	 * ExtentManager.getInstance(); }
	 * 
	 * @BeforeMethod public void initializeTestBaseSetup(Method method) { try {
	 * ExtentTestManager.startTest(method.getName()); setDriver(browser,
	 * utilMobile.getConfigValue("appUrl"));
	 * 
	 ...
	 * 
	 * 
	 * protected String getStackTrace(Throwable t) { StringWriter sw = new
	 * StringWriter(); PrintWriter pw = new PrintWriter(sw);
	 * t.printStackTrace(pw); return sw.toString(); }
	 * 
	 * @AfterMethod public void afterEachTestMethod(ITestResult result) {
	 * ExtentTestManager.getTest().getTest().setStartedTime(getTime(result.
	 * getStartMillis())); // new
	 * ExtentTestManager.getTest().getTest().setEndedTime(getTime(result.
	 * getEndMillis())); // new
	 * 
	 ...
	 * else if (result.getStatus() == 2) { String path =
	 * utilMobile.getscreenshot(driver, result.getName()); //
	 * ExtentTestManager.getTest().log(LogStatus.FAIL,
	 * getStackTrace(result.getThrowable())); // new String image =
	 * ExtentTestManager.getTest().addScreenCapture(path);
	 * ExtentTestManager.getTest().log(LogStatus.FAIL,getStackTrace(result.
	 * getThrowable())); ExtentTestManager.getTest().log(LogStatus.FAIL,image);
	 * driver.get(path); } else if (result.getStatus() == 3) {
	 ...
	 * 
	 * @AfterSuite public void generateReport() { extent.close(); }
	 * 
	...
	public void initializeTestBaseSetup(Method method) {
		try {
			...
			setDriver(browser, utilMobile.getConfigValue("appUrl"));
			Thread.sleep(5000);
